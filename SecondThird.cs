...
namespace Lesson6.SecondThird
{
    ...
    class PassPost 
    {
        public static void Main()
        {
            ...
            {
                Console.WriteLine("Серия паспорта: {0}; Номер паспорта: {1}", 
                                    item.PassSeries, item.PassNumber);
            }

            Console.WriteLine("Введите паспортный номер, чтобы узнать ФИО человека.");
            int searchNumber = Convert.ToInt32(Console.ReadLine());
            Passport found = pass.Find(item => item.PassNumber == searchNumber);

            Console.WriteLine("ФИО: {0}, номер паспорта: {1}", found.FIO, found.PassNumber);
        }
